...
VOICE_REQUIREMENTS_FILE = PROJECT_DIR / "CosyVoice/requirements.txt"
...
def install_requirements():
    ...
    #     print("[INFO] 检测到 NVIDIA GPU，将安装 CUDA 12.8 版 PyTorch (cu128)。")
    # else:
    ...
    if VOICE_REQUIREMENTS_FILE.exists():
        print(f"[INFO] 安装 CosyVoice requirements.txt: {VOICE_REQUIREMENTS_FILE}")
        try:
            subprocess.run(pip_cmd + ["install", "-r", str(VOICE_REQUIREMENTS_FILE)], check=True)
        except subprocess.CalledProcessError as e:
        ...
        print(f"[WARN] 未找到 {VOICE_REQUIREMENTS_FILE}，跳过依赖安装。")
...
def run_script():
    py_path = venv_python()
    script_path = PROJECT_DIR / "run_server.py"
