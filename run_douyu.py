...
def find_python310_cmd() -> list[str]:
    """
    找到“能运行出 3.10.x”的命令行（带参数），例如：
    - ["python3.10"]
    - ["py", "-3.10"]
    - ["python311"]
    找不到就退出提示安装 3.10。
    ...
    for cmd in candidates:
        try:
            ...
            if "3.10" in text:
                print(f"[INFO] 使用 Python 3.10 命令: {' '.join(cmd)}")
                return cmd
        except Exception:
            continue

    print("\n[ERROR] 未找到 Python 3.10 可用命令，无法创建 XTTS 兼容环境。")
    print("请先安装 Python 3.10： https://www.python.org/downloads/release/python-3119/")
    if is_windows():
        input("按回车退出...")
    sys.exit(1)


PY311_CMD = find_python310_cmd()
...
def ensure_venv():
    """
    1. 如果没有 venv：用 3.10 创建
    2. 如果已有 venv：检查里面的 python 版本
       - 不是 3.10 → 删除整个 venv，重新用 3.10 创建
    """
    need_create = False
    ...
    if not VENV_DIR.exists():
        ...
        if not py.exists():
            ...
            try:
                ...
                if "3.10" not in out.decode("utf-8"):
                    print("[WARN] 当前 venv 不是 Python 3.10，将重新创建。")
                    need_create = True
            except Exception:
                need_create = True
    ...
    if need_create:
        ...
        print("[INFO] 使用 Python 3.10 创建新的虚拟环境 venv ...")
        # 这里非常关键：保留完整命令，比如 ["py", "-3.10", "-m", "venv", "venv"]
